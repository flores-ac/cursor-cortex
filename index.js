  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: 'Query the GraphQL Knowledge Hub API for structured knowledge discovery across projects, branches, and tacit knowledge',
        inputSchema: {
          type: 'object',
          properties: {
            query: { 
              type: 'string', 
              description: 'GraphQL query string (e.g., "{ projects { name stats { averageCompletenessScore } } }")' 
            },
            variables: { 
              type: 'object', 
              description: 'Optional GraphQL variables as JSON object (e.g., {"projectName": "cursor-cortex"})' 
            },
            endpoint: { 
              type: 'string', 
      ...
      },
      {
        ...
        description: 'Generate GraphQL context from selected Cursor-Cortex knowledge for team onboarding and project discovery',
        inputSchema: {
          type: 'object',
          properties: {
            projectNames: { 
              type: 'array', 
              items: { type: 'string' },
              description: 'Array of project names to export (e.g., ["cursor-cortex", "metrics_library_v2"])' 
            },
            includeBranches: { 
              type: 'array', 
              items: { type: 'string' },
              description: 'Specific branch names to include (optional - if not provided, includes all branches)' 
            },
            includeTypes: { 
              type: 'array', 
              items: { type: 'string' },
              description: 'Data types to include: branch-notes, context, tacit-knowledge (default: all)' 
            },
            graphqlEndpoint: { 
              type: 'string', 
              description: 'GraphQL endpoint URL (defaults to http://localhost:4000/graphql)' 
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `# 🔍 GraphQL Knowledge Hub Query Results
...
✅ **Query executed successfully via GraphQL Knowledge Hub**
🌐 **Endpoint**: ${endpoint}
...
`,
        ...
        let errorMessage = 'Failed to execute GraphQL query';
        
        if (error.code === 'ECONNREFUSED') {
          errorMessage = `GraphQL Knowledge Hub server is not running at ${endpoint}. Start it with: npm start`;
        } else if (error.response?.status === 400) {
          errorMessage = `GraphQL syntax error: ${error.response.data?.errors?.[0]?.message || 'Invalid query'}`;
...
💡 **Troubleshooting**:
1. Ensure GraphQL server is running: \`node server.cjs\`
2. Check endpoint URL is correct
3. Validate GraphQL query syntax
4. Check if server is accessible at ${endpoint}
`,
            },
          ],
        };
      }
    } else if (name === 'generate_context_graph') {
      const { 
        projectNames, 
        includeBranches = [], 
        includeTypes = ['branch-notes', 'context', 'tacit-knowledge'],
        graphqlEndpoint = 'http://localhost:4000/graphql'
      } = toolArgs;
      
      try {
        console.error(`🚀 Generating GraphQL context for ${projectNames.length} projects...`);
        ...
        for (const projectName of projectNames) {
          console.error(`📊 Processing project: ${projectName}`);
          
          try {
            const projectResult = { name: projectName, status: 'processing', data: {} };
            
            // Get project context if requested
            if (includeTypes.includes('context')) {
              try {
                const contextResult = await readProjectContext(projectName, projectName);
                
                projectResult.data.context = {
                  status: 'captured',
                  size: contextResult ? contextResult.length : 0
                };
                
              } catch (contextError) {
                console.error(`⚠️ Context capture failed for ${projectName}: ${contextError.message}`);
                projectResult.data.context = { status: 'failed', error: contextError.message };
              }
            }
            
            // Get branch notes if requested
            if (includeTypes.includes('branch-notes')) {
              const branchesToProcess = includeBranches.length > 0 ? includeBranches : ['main', 'feature/graphql-knowledge-api'];
              projectResult.data.branches = [];
              
              for (const branchName of branchesToProcess) {
                try {
                  // Duplicate logic from read_branch_notes MCP tool
                  const branchNotePath = getBranchNotePath(projectName, branchName);
                  console.error(`🔍 Trying to read: ${branchNotePath}`);
                  let branchNoteContent;
                  
                  try {
                    branchNoteContent = await fs.readFile(branchNotePath, 'utf-8');
                    console.error(`✅ File read successfully: ${branchNoteContent.length} chars`);
                  } catch (readError) {
                    console.error(`⚠️ Branch file not found: ${branchNotePath} - Error: ${readError.message}`);
                    projectResult.data.branches.push({
                      name: branchName,
                      status: 'not_found',
                      error: `Branch note file does not exist: ${readError.message}`
                    });
                    continue;
                  }
                  
                  if (branchNoteContent && typeof branchNoteContent === 'string') {
                    const entryCount = (branchNoteContent.match(/## \d{4}-\d{2}-\d{2}/g) || []).length;
                    
                    projectResult.data.branches.push({
                      name: branchName,
                      status: 'captured',
                      size: branchNoteContent.length,
                      entries: entryCount
                    });
                  ...
                  console.error(`⚠️ Branch capture failed for ${projectName}/${branchName}: ${branchError.message}`);
                  projectResult.data.branches.push({
                    name: branchName,
                    status: 'failed',
            ...
            // Get tacit knowledge if requested
            if (includeTypes.includes('tacit-knowledge')) {
              try {
                const tacitResult = await readTacitKnowledge(projectName, '', false, 0.4);
                
                projectResult.data.tacitKnowledge = {
                  status: 'captured',
                  documents: tacitResult ? (tacitResult.match(/### /g) || []).length : 0
                };
                
              } catch (tacitError) {
                console.error(`⚠️ Tacit knowledge capture failed for ${projectName}: ${tacitError.message}`);
                projectResult.data.tacitKnowledge = { status: 'failed', error: tacitError.message };
              }
            }
            ...
            console.error(`❌ Project processing failed for ${projectName}: ${projectError.message}`);
            results.push({
              name: projectName,
              status: 'failed',
        ...
        return {
          content: [
            {
              type: 'text',
              text: `# 🚀 GraphQL Context Generation Complete

## Summary
- **Projects Processed**: ${processedProjects}/${projectNames.length}
- **Branches Captured**: ${processedBranches}
...
${results.map(project => {
  if (project.status === 'completed') {
    return `### ✅ ${project.name}
${project.data.context ? `- Context: ${project.data.context.status} (${project.data.context.size || 0} chars)` : ''}
${project.data.branches ? `- Branches: ${project.data.branches.filter(b => b.status === 'captured').length} captured` : ''}
${project.data.tacitKnowledge ? `- Knowledge Docs: ${project.data.tacitKnowledge.documents || 0} documents` : ''}`;
  } else {
    return `### ❌ ${project.name}
...
${includeTypes.map(type => `- ✅ ${type}`).join('\n')}

## Next Steps
1. **Test GraphQL Query**:
\`\`\`graphql
query CheckGeneratedContext {
  projects {
    name
    description
    stats {
...
2. **Verify Team Context**:
\`\`\`graphql
query SearchTeamKnowledge($searchTerm: String!) {
  searchKnowledge(query: $searchTerm) {
    totalResults
    results {
      type
      title
...
✅ **Team knowledge context is ready for GraphQL discovery!**
`,
            },
          ],
        };
        ...
        return {
          ...
          content: [
            {
              type: 'text',
              text: `❌ **GraphQL Context Generation Failed**
...
💡 **Troubleshooting**:
1. Ensure GraphQL server is running at ${graphqlEndpoint}
2. Check that projects exist in Cursor-Cortex: \`mcp_cursor-cortex_list_all_branch_notes\`
3. Verify MCP tools are working: \`mcp_cursor-cortex_read_project_context\`
